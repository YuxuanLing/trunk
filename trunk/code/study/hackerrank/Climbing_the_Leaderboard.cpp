...
vector<int> climbingLeaderboard(vector<int> scores, vector<int> alice) {
	...
	for(int i = 0 ; i < alice.size(); i++)
	{
	    ...
	    int sz = scores.size(), j, rank;
		for(j = 0; j < sz; j++)
		{			
			if(alice_score >= scores[j])
			{
				scores.insert(scores.begin() + j, alice_score);
				scores.erase(scores.begin() + j + 1, scores.end());
                break;				
			}			
		}
		
		if(j == sz)scores.push_back(alice_score);
		rank = findRank(scores);
...
int main()
{
	...
	//for (int i = 0; i < scores_temp.size(); i++) {
		//cout << scores_temp[i]<<endl;
