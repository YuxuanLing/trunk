...
int main(int argc, char* argv[]){
    ...
    char* source = NULL;
    size_t srcLen;
    const char* filename = "sharedBlockingKernel.cl";
    ...
    const char *options = "-DT=float -DBX=2 -DBY=2 -DfilterSize=5 -DBS=16";
    cl_program program;
    ...
    const char* kernelName = "convolutionConstantSharedUnroll";
    cl_kernel kernel;
    ...
    size_t gx = ((numCols-4)/2+local_work_size[0]-1)/(local_work_size[0])*(local_work_size[0]);
    size_t gy = ((numRows-4)/2+local_work_size[1]-1)/(local_work_size[1])*(local_work_size[1]);
...
printf("%.3f ms\n", time);
    ...
    assert(CL_SUCCESS == clEnqueueReadBuffer(commandQueue, out, CL_TRUE, 0, sizeof(float) * size, h_out, 1, NULL, NULL));
