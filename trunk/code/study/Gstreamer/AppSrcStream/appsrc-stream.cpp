...
{
	GST_WARNING("seek to offset %" G_GUINT64_FORMAT, position);
	app->offset = position;
...
bus_message(GstBus * bus, GstMessage * message, App * app)
{
	...
	case GST_MESSAGE_ERROR:
		g_warning("received error, quit main loop");
	...
	case GST_MESSAGE_EOS:
		...
		break;
	default:
		g_message("default msg recv %s", GST_MESSAGE_TYPE_NAME(message));
		break;
	}
	return TRUE;
}
